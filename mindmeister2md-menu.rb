...
REXML::Document.new(listbody).elements.each("rsp/maps/map") { |e|
  ...
  menu << {'item_number' => item_number, 'title' => mtitle, 'id' => mid }
  item_number += 1
}
menu.each {|item|
  puts "#{item['item_number']}: #{item['title']}    [ #{item['id']} ]"
}
map_id = nil
print "Which map? "
sel = STDIN.gets
if sel =~ /^\d+$/ && sel.to_i > 0 && sel.to_i < menu.length + 1
  map_id = menu[sel.to_i - 1]['id']
else
  puts "Sorry, that's not an option."
end
Process.exit(-1) if map_id.nil?
...
def print_level ( node, spaces)
  ...
  node.children.each { |n|
    o += print_level(n, spaces + 1)
  }
  o
end

output = print_level(root, 0)
...
puts "Markdown written to #{filename}"
